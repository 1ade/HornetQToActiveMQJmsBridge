...
public class JMSBridgeExample
{
	public static void main(final String[] args) throws Exception
	{
		...
		InitialContext targetContext = JMSBridgeExample.createContextAMQ(targetServer);

		// create JNDI's for hornetQ
		Hashtable<String, String> sourceJndiParams = createJndiParams(sourceServer);
      
		// create JNDI's for activeMQ
		Hashtable<String, String> targetJndiParams = createJndiParamsForAMQ(targetServer);
		...
		JMSBridge jmsBridge = new JMSBridgeImpl(
				 ...
				 new JNDIConnectionFactoryFactory(targetJndiParams, "XAConnectionFactory"),
				 new JNDIDestinationFactory(sourceJndiParams, "/source/topic"),
				 new JNDIDestinationFactory(targetJndiParams, "MyQueue"),
				 null,
				 null,
				 "admin",
				 "admin",
		...
		try
		{
			...
			// Step 7. Lookup the *target* ActiveMQ JMS resources
			ConnectionFactory targetConnectionFactory = (ConnectionFactory)targetContext.lookup("ConnectionFactory");

			Queue targetQueue = (Queue)targetContext.lookup("MyQueue");
			...
			TextMessage messageReceived = (TextMessage)targetConsumer.receive();
	...
	private static InitialContext createContextAMQ(String targetServer) throws Exception
	{
		Hashtable<String, String> jndiProps = createJndiParamsForAMQ(targetServer);
		return new InitialContext(jndiProps);
	}
	private static Hashtable<String, String> createJndiParamsForAMQ(String targetServer)
	{
		Hashtable<String, String> jndiProps = new Hashtable<String, String>();
		jndiProps.put("java.naming.provider.url", targetServer);
		jndiProps.put("java.naming.factory.initial", "org.apache.activemq.jndi.ActiveMQInitialContextFactory");
		jndiProps.put("queue.MyQueue", "sample.queue");
		return jndiProps;
	}
}
